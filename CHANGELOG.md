...
- **内容页标题与左上角标间距（S27 P4/P6/P11，ADR-077）**：模板左上角 `⇱` 双线角标与档案标题锚点仅差 0.01in，
  标题视觉上被角标挤压。现在 `content` 页标题从档案锚点向右下各让 8px（1/12in），并按档案自己的字号阶梯降一号
  （参考档案 36→34pt）；卡片版式保持 ADR-075 的等宽列。
- **符合性审计同步（ADR-077）**：title 字号/锚点期望值带上上述让位与降档；toc 标题锚点不再参与比较（ADR-073）；
  内容页列数/卡间距改从卡片面板测量并接受 `columns`–`columnsMax` 区间；20pt 强调色卡标题不再计入 body 模式。
- **未启用生图时预留插图区（设计语言 §4/§6）**：默认路径不调用 `image-gen`，内容页须留出 ≥1/4 页面积的插图空位
  （或三列卡片只铺两列），留给 `user`/`office` 素材或用户后补，不得用文字铺满。
- 设计语言参考文件预算从 2000 提到 2250 tokens（skill audit 25 files、111,756/120,000，0 error）。
