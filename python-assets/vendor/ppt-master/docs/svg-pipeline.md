...
```bash
uvx ppt-master svg-authoring-view <svg-file-or-directory> -o <output-dir> \
...
```bash
uvx ppt-master svg-authoring-view <authoring-dir> \
...
```bash
uvx ppt-master pptx-to-svg source.pptx \
  -o /path/to/workspace --inheritance-mode both --roundtrip
uvx ppt-master svg-to-pptx /path/to/workspace \
...
Before export, run `uvx ppt-master svg-quality-check <workspace> --roundtrip --json`
...
is owned by [`shared-standards-core.md`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/references/shared-standards-core.md) §1.5, with
authoring guidance in
[`native-shape-authoring.md`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/references/native-shape-authoring.md).
...
inline-smoke convention from [`code-style.md`](https://github.com/elvisw/ppt-master/blob/main/docs/rules/code-style.md)
...
```bash
uvx ppt-master compact-svg-styles <svg-file-or-directory>
uvx ppt-master compact-svg-styles <svg-file-or-directory> --inplace
```
...
```bash
uvx ppt-master extract-svg-assets <layered_svg_dir> \
  --icons-dir <icons_dir> --icon-namespace imported \
  --inplace --id-prefix layered
uvx ppt-master extract-svg-assets <flat_svg_dir> \
  --icons-dir <icons_dir> --icon-namespace imported \
...
```bash
uvx ppt-master stamp-native-fallbacks <svg-or-directory> --write
```
...
```bash
uvx ppt-master extract-svg-pictures \
...
`uvx ppt-master svg-to-pptx <project_path> --no-notes` for the final
...
[`generate-pptx.md`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/workflows/generate-pptx.md) Step 7.
...
[`native-formula.md`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/references/native-formula.md). A standalone block
...
uvx ppt-master svg-to-pptx <project_path> --image-sizing display --image-scale 2 --image-quality 85
# Force original image bytes:
uvx ppt-master svg-to-pptx <project_path> --no-image-optimize
uvx ppt-master svg-to-pptx <project_path> --native-charts-and-tables
uvx ppt-master svg-to-pptx <project_path> --pptx-structure structured  # deck/layout template override
uvx ppt-master svg-to-pptx <project_path> --pptx-structure flat  # free-design/brand-only override
...
uvx ppt-master svg-to-pptx <pptx_import_output> --roundtrip
...
uvx ppt-master svg-to-pptx <project_path> --reflow-text  # opt-in PowerPoint reflow
uvx ppt-master svg-to-pptx <project_path> --no-merge    # one text frame per visual line
uvx ppt-master svg-to-pptx <project_path> --recorded-narration audio
uvx ppt-master svg-to-pptx <project_path> --recorded-narration audio --animation-config animations.json
uvx ppt-master svg-to-pptx <project_path> --recorded-narration audio --no-animations
```
...
[`quick-generate`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/workflows/profiles/quick-generate.md) profile after the
...
[`generate-audio`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/workflows/stages/generate-audio.md) stage. It owns voice
...
- Legacy structured/template contracts using `baseline`, `template`, `preserve`, `layout_strategy`, `data-pptx-layout-kind`, `distilled`/`utility`, direct atomic placeholders, or incomplete Master identity are rejected with a pointer to [`create-template`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/workflows/create-template.md). Create a new workspace and generate new structured SVG pages; do not upgrade the existing project in place. Explicit flat free-design/brand-only projects intentionally omit Master identity.
...
- For chrome defaults, static role/placeholder overrides, and structural exclusions, see [`animations.md`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/references/animations.md) §5
...
Checker and exporter behavior behind [`pptx-structure-interface.md`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/references/pptx-structure-interface.md) §2.
...
Pure render-and-validate tool for the [`visual-review`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/workflows/stages/visual-review.md) stage; it never edits SVGs and reads no rubric rule.

```bash
uvx ppt-master visual-review <project_path> [--pages <token> ...] [--server-url http://127.0.0.1:<P>]
```
...
uvx ppt-master svg-quality-check projects/project --stage early
uvx ppt-master svg-quality-check projects/project --stage final --json
uvx ppt-master svg-quality-check projects/project --canonical-authoring --stage final --json
...
Used by the [`verify-charts`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/workflows/stages/verify-charts.md) stage for chart objects whose geometry reduces to repeated direct calculations (`decomposable-calc` / `partial-calc`), a closed formula (`formula-verify`), or inspection only (`manual-verify`). Every recipe produces one receipt line; a page that cannot be reduced cleanly is marked `manual-verify` with the reason, never dropped.
...
uvx ppt-master svg-position-calc calc bar --data "Q1:30,Q2:..." --area "x_min,100,x_max,500" --bar-width 80 --value-range=0,axis_max
uvx ppt-master svg-position-calc calc bar --data "Q1:20,Q2:..." --area "x_min,100,x_max,<500 - bottom_height_px>" --bar-width 80 --value-range=0,axis_max
```
...
```bash
uvx ppt-master svg-position-calc calc line --data "42:0.5,55:1.5,37:2.5" --area "100,100,700,460" --x-range=0,100 --y-range=0,3
uvx ppt-master svg-position-calc calc line --data "68:0.5,71:1.5,49:2.5" --area "100,100,700,460" --x-range=0,100 --y-range=0,3
```
...
[`shared-standards-core.md`](https://github.com/elvisw/ppt-master/blob/main/skills/ppt-master/references/shared-standards-core.md), with
