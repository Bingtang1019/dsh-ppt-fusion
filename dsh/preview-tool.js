// Adapted for dsh-ppt-fusion from @liustack/pptwise's dsh/preview-tool.js
// (MIT; see NOTICE). Tool/route identity, the plugin home, the snapshot step and the
// CLI commands are the fusion's; the card protocol is unchanged.
//
...
// log: 34 top-level `run_code` calls, `dsh_ppt_preview` never once among
 ...
 * caused. Usually, not always — the root follows `DSH_PPT_PREVIEW_HOME`, so a deck
...
function missingPage(message) {
  return noticePage(
    ...
    `Rendered decks stay in <code>${escapeHtml(previewRoot())}</code> until you delete them. ` +
      'Run <code>dsh_ppt_preview</code> again to rebuild this one.',
  )
}
...
function damagedPage(message) {
  return noticePage(
    ...
    "The rendered pages may still be there — it is the file describing them that this version cannot read. " +
      'Run <code>dsh_ppt_preview</code> again to rebuild it.',
  )
}
...
export const TOOL_NAME = 'dsh_ppt_preview'

export const PREVIEW_ROUTE = '/dsh-ppt/preview'
 ...
 */
export const ROUTE_HEADER = 'x-dsh-ppt-preview'
...
//
//  1. The records lived in `$TMPDIR/dsh-ppt-previews/<sha256(cliPath)[0:16]>/`.
...
/** Directory under `$DSH_PPT_PREVIEW_HOME` that holds every preview this plugin has kept. */
const PREVIEW_DIR = 'previews'

/**
 * Root of everything this module writes.
 *
 * `DSH_PPT_PREVIEW_HOME` wins; the default is `~/.dsh-ppt`. Empty string is unset.
 * Resolved to an absolute path: a relative root would make every path here depend
 * on where the harness happened to be started.
 */
...
function resolvePluginHome(opts = {}) {
  ...
  const current = nonemptyEnv(env, 'DSH_PPT_PREVIEW_HOME')
  if (current !== undefined) return current
  return join(homeFn(), '.dsh-ppt')
}
 ...
 */
const OWNER_MARKER = '.dsh-ppt-preview-owner'
...
async function locateDeckDir(target) {
  ...
  // `dirname(previewRoot())` rather than a second hard-coded `~/.dsh-ppt`, so
  // this follows `DSH_PPT_PREVIEW_HOME` the way the fusion CLI's `decksRoot()` does. The
...
async function captureSnapshot(cliPath, target, outDir, signal) {
  ...
  // The fusion deck keeps its pptwise IR at the manifest's `pptwiseIr`; copy that
  // file instead of running an assemble step (the fusion CLI has none).
  const root = deckDir ?? resolve(target)
  const manifest = JSON.parse(await readFile(join(root, 'deck.fusion.json'), 'utf8'))
  const irName = typeof manifest.pptwiseIr === 'string' ? manifest.pptwiseIr : 'deck.ir.json'
  await copyFile(join(root, irName), snapshot)
  await inlineLocalImages(snapshot)
  if (deckDir) {
...
function modelSummary(value) {
  ...
  bits.unshift(`dsh-ppt-preview:${value.previewId}`)
...
export function createPreviewService(cliPath) {
  ...
  async function render(outDir, target, exec) {
    ...
    try {
      ...
      // The fusion render takes a deck directory and a deck-relative output and
      // merges the deep pages; copy the finished package into the preview cache
      // so the download route serves a file.
      const deckRelative = join('out', basename(pptxPath))
      await runCli(cliPath, ['render', target, '-o', deckRelative], exec?.signal)
      await copyFile(join(resolve(target), deckRelative), pptxPath)
    } catch (error) {
  ...
  const tool = {
    ...
    output: {
      ...
      presentationMeta(_args, value) {
        return { card: 'dsh-ppt-preview', previewId: value.previewId, bundle: value.bundle }
      },
    },
