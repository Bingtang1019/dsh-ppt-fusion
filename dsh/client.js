// Adapted for dsh-ppt-fusion from @liustack/pptwise's dsh/client.js (MIT; see NOTICE).
// Browser half of the dsh-ppt-fusion dsh plugin: the deck preview card.
//
// The host half (`./preview-tool.js`) registers `dsh_ppt_preview` and puts
...
window.__ModuleLoader__.load({
  id: 'dsh-ppt-flashmade',
  factory: (require) => {
    var module = { exports: {} }
    var exports = module.exports

    var TOOL_NAME = 'dsh_ppt_preview'
    ...
    function previewHtmlUrl(previewId, startPage) {
      var url = '/dsh-ppt/preview/' + previewId + '/html'
    ...
    function previewBundleUrl(previewId) {
      return '/dsh-ppt/preview/' + previewId
    }
    ...
    var ROUTE_HEADER = 'x-dsh-ppt-preview'
     ...
     * not the only one: `DSH_PPT_PREVIEW_HOME` decides that root, so a deck written
    ...
    var MISSING_HINT =
      ...
      'DSH_PPT_PREVIEW_HOME. Run dsh_ppt_preview again to rebuild it.'

    /** What the card says when it could not reach the route at all. */
    var UNREACHABLE_HINT =
      'Could not reach the dsh-ppt preview route. The deck may still be there — try again.'

    /** What the card says when the route answered, but refused. */
    var REFUSED_HINT =
      'The dsh-ppt preview route refused this request. The deck may still be there, ' +
    ...
    var DAMAGED_HINT =
      ...
      'The rendered pages may still be on disk — run dsh_ppt_preview again to rebuild it.'
    ...
    function bundleOf(block) {
      ...
      for (var i = 0; i < candidates.length; i++) {
        var c = candidates[i]
        if (c && c.card === 'dsh-ppt-preview' && c.bundle && Array.isArray(c.bundle.pages)) return c.bundle
    ...
    function PreviewCard(react) {
       ...
       * the exit.
       */
      var MODAL_STYLE_ID = 'dsh-ppt-modal-style'
      var MODAL_BTN_BASE =
        ...
        return h(
          ...
          h(
            ...
            h('iframe', {
              ref: frameRef,
              src: props.src,
              title: 'dsh-ppt-fusion deck preview',
              style: {
      ...
      function ExportButton(props) {
        ...
        function save() {
          ...
          fetch('/dsh-ppt/preview/' + props.previewId + '/pptx')
            .then(function (res) {
      ...
      return function DshPptPreviewCard(props) {
    ...
    function registerCard(ctx) {
      ...
      if (!ctx.slots || typeof ctx.slots.inject !== 'function') {
        console.error('[dsh-ppt-fusion] preview card skipped: no slot registry on this context')
        return
      }
      var react
      try {
        ...
        console.error('[dsh-ppt-fusion] preview card skipped: ' + error)
        return
      }
