...
| S36 | 渲染门禁有效 | `dsh-ppt audit --rendered` 在 reference-quality deck 上跑通 `render` 源（PowerPoint + LibreOffice 两引擎 12 页）；规则：page-count / content-loss（空白或不可解码）/ off-page（几何越界 + 外带墨迹 warning）/ overlap（几何）/ contrast（盒内最深墨色 vs 页面背景，<18pt 4.5:1、≥18pt 3:1）/ chrome（声明页码带）/ tofu / overflow（warning）/ parity（warning）；`--require-rendered` 把缺快照升级为 `render-snapshot-missing`；`render-audit.test.ts` 钉住空白/有墨、页数、几何重合/越界与 chrome 带（ADR-082）。**首个真实捕获：P2 目录页标题渲染 0% 墨迹 + 左右卡文本框跨整幅重叠（用户 S27 反馈的"字被卡片挡住"）** | 无（本轮无） | V10 Part A |
